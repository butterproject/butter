...
mkdir -p $cwd/$package_name/opt/Butter #app files
mkdir -p $cwd/$package_name/usr/share/icons #icon

### COPY FILES
#base
cp -r build/cache/$arch/$nw/locales $cwd/$package_name/opt/Butter/
cp build/cache/$arch/$nw/icudtl.dat $cwd/$package_name/opt/Butter/
cp build/cache/$arch/$nw/libffmpegsumo.so $cwd/$package_name/opt/Butter/
cp build/cache/$arch/$nw/nw $cwd/$package_name/opt/Butter/Butter
cp build/cache/$arch/$nw/nw.pak $cwd/$package_name/opt/Butter/

#src
cp -r src $cwd/$package_name/opt/Butter/
cp package.json $cwd/$package_name/opt/Butter/
cp LICENSE.txt $cwd/$package_name/opt/Butter/
cp CHANGELOG.md $cwd/$package_name/opt/Butter/

#node_modules
cp -r node_modules $cwd/$package_name/opt/Butter/node_modules
...
### CLEAN
shopt -s globstar
cd $cwd/$package_name/opt/Butter
...
Name=Butter
Exec=/opt/Butter/Butter
Icon=butter
MimeType=application/x-bittorrent;x-scheme-handler/magnet;
StartupNotify=false
Categories=AudioVideo;Video;Network;Player;P2P;
Type=Application
" > $cwd/$package_name/usr/share/applications/butter.desktop
...
Installed-Size: $size
Depends:
Maintainer: Butter Project <butter@xaiki.net>
Description: Butter
...
Upstream-Contact: Butter Project <butter@xaiki.net>
Source: https://github.com/butterproject/butter

Files: *
Copyright: © 2015, Butter Project and the contributors <butter@xaiki.net>
License: GPL-3+
...
if [ -x /usr/bin/desktop-file-install ]; then
	desktop-file-install /usr/share/applications/butter.desktop
else
	chmod +x /usr/share/applications/butter.desktop
fi

# set permissions for updates
if [ -e /opt/Butter/Butter ]; then
	chmod -R 777 /opt/Butter
fi

if [ ! -e /lib/$(arch)-linux-gnu/libudev.so.1 ]; then
	ln -s /lib/$(arch)-linux-gnu/libudev.so.0 /opt/Butter/libudev.so.1
	sed -i 's,Exec=,Exec=env LD_LIBRARY_PATH=/opt/Butter ,g' /usr/share/applications/butter.desktop
fi
...
#remove desktop
rm -rf /usr/share/applications/butter.desktop
...
if [ \"\$1\" = purge ]; then
	rm -rf \$HOME/.config/Butter
fi
" > $cwd/$package_name/DEBIAN/postrm

### PERMISSIONS
chmod 0644 $cwd/$package_name/usr/share/applications/butter.desktop
