...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        exec: {
            ...
            createDmg: {
                cmd: 'dist/mac/yoursway-create-dmg/create-dmg --volname "Butter ' + currentVersion + '" --background ./dist/mac/background.png --window-size 480 540 --icon-size 128 --app-drop-link 240 370 --icon "Butter" 240 110 ./build/releases/Butter/mac/Butter-' + currentVersion + '-Mac.dmg ./build/releases/Butter/mac/ || echo "Create dmg failed, likely caused by not being run on mac, continuing"'
            },
            createWinInstall: {
        ...
        shell: {
            ...
            setexecutable: {
                command: function () {
                    if (host.linux || host.mac) {
                        return [
                            'pct_rel="build/releases/Butter"',
                            'chmod -R +x ${pct_rel}/mac/Butter.app || : ',
                            'chmod +x ${pct_rel}/linux*/Butter/Butter || : '
                        ].join(' && ');
                    } else {
            ...
            packageLinux64: {
                command: function () {
                    if (host.linux || host.mac) {
                        return [
                            'cp build/cache/linux64/<%= nodewebkit.options.version %>/icudtl.dat build/releases/Butter/linux64/Butter',
                            'cp -r build/cache/linux64/<%= nodewebkit.options.version %>/locales build/releases/Butter/linux64/Butter',
                            'cp dist/linux/linux-installer build/releases/Butter/linux64/Butter/install',
                            'cp dist/linux/butter.png build/releases/Butter/linux64/Butter',
                            'cd build/releases/Butter/linux64/Butter',
                            'chmod +x install',
                            'tar --exclude-vcs -c . | $(command -v pxz || command -v xz) -T8 -7 > "../Butter-' + currentVersion + '-Linux-64.tar.xz"',
            ...
            packageLinux32: {
                command: function () {
                    if (host.linux || host.mac) {
                        return [
                            'cp build/cache/linux32/<%= nodewebkit.options.version %>/icudtl.dat build/releases/Butter/linux32/Butter',
                            'cp -r build/cache/linux32/<%= nodewebkit.options.version %>/locales build/releases/Butter/linux32/Butter',
                            'cp dist/linux/linux-installer build/releases/Butter/linux32/Butter/install',
                            'cp dist/linux/butter.png build/releases/Butter/linux32/Butter',
                            'cd build/releases/Butter/linux32/Butter',
                            'chmod +x install',
                            'tar --exclude-vcs -c . | $(command -v pxz || command -v xz) -T8 -7 > "../Butter-' + currentVersion + '-Linux-32.tar.xz"',
            ...
            packageWin: {
                command: function () {
                    if (host.linux || host.mac) {
                        return [
                            'cp build/cache/win/<%= nodewebkit.options.version %>/icudtl.dat build/releases/Butter/win/Butter',
                            'cp -r build/cache/win/<%= nodewebkit.options.version %>/locales build/releases/Butter/win/Butter',
                            'cd build/releases/Butter/win/Butter',
                            'tar --exclude-vcs -c . | $(command -v pxz || command -v xz) -T8 -7 > "../Butter-' + currentVersion + '-Win.tar.xz"',
            ...
            packageMac: {
                command: function () {
                    if (host.linux || host.mac) {
                        return [
                            'cd build/releases/Butter/mac/',
                            'tar --exclude-vcs -c Butter.app | $(command -v pxz || command -v xz) -T8 -7 > "Butter-' + currentVersion + '-Mac.tar.xz"',
        ...
        compress: {
            linux32: {
                options: {
                    mode: 'tgz',
                    archive: 'build/releases/Butter/linux32/Butter-' + currentVersion + '-Linux-32.tar.gz'
                },
                expand: true,
                cwd: 'build/releases/Butter/linux32/Butter',
                src: '**',
                dest: 'Butter'
            },
            linux64: {
                options: {
                    mode: 'tgz',
                    archive: 'build/releases/Butter/linux64/Butter-' + currentVersion + '-Linux-64.tar.gz'
                },
                expand: true,
                cwd: 'build/releases/Butter/linux64/Butter',
                src: '**',
                dest: 'Butter'
            },
            mac: {
                options: {
                    mode: 'tgz',
                    archive: 'build/releases/Butter/mac/Butter-' + currentVersion + '-Mac.tar.gz'
                },
                expand: true,
                cwd: 'build/releases/Butter/mac/',
                src: '**',
                dest: ''
            },
            windows: {
                options: {
                    mode: 'tgz',
                    archive: 'build/releases/Butter/win/Butter-' + currentVersion + '-Win.tar.gz'
                },
                expand: true,
                cwd: 'build/releases/Butter/win/Butter',
                src: '**',
                dest: 'Butter'
            }
        },
        ...
        clean: {
            releases: ['build/releases/Butter/**'],
            css: ['src/app/themes/**'],
